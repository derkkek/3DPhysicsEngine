...
namespace Cacti
{
	...
	Vec3 NormalDirection(const tri& tri, const std::vector< point_t >& points) {
	...
	float SignedDistanceToTriangle(const tri& tri, const Vec3& pt, const std::vector< point_t >& points) {
	...
	int ClosestTriangle(const std::vector< tri >& triangles, const std::vector< point_t >& points) {
	...
	bool HasPoint(const Vec3& w, const std::vector< tri > triangles, const std::vector< point_t >& points) {
	...
	int RemoveTrianglesFacingPoint(const Vec3& pt, std::vector< tri >& triangles, const std::vector< point_t >& points) {
		int numRemoved = 0;
		for (int i = 0; i < triangles.size(); i++) {
			const tri& tri = triangles[i];
	...
	void FindDanglingEdges(std::vector< Edge >& danglingEdges, const std::vector< tri >& triangles) {
		...
		for (int i = 0; i < triangles.size(); i++) {
			...
			for (int j = 0; j < triangles.size(); j++) {
				...
				const Cacti::tri& tri2 = triangles[j];
